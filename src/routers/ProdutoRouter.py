...
async def get_produto_por_id(id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(ProdutoModel).where(ProdutoModel.id_produto == id))
...
async def post_produto(corpo: Produto, db: AsyncSession = Depends(get_db)):
    novo = ProdutoModel(
        nome           = corpo.nome,
...
async def put_produto(id: int, corpo: Produto, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(ProdutoModel).where(ProdutoModel.id_produto == id))
    produto = result.scalars().first()
    if not produto:
        raise HTTPException(status_code=404, detail="Produto não encontrado")
    ...
    return {"id_produto": produto.id_produto, "nome": produto.nome,
            "descricao": produto.descricao, "valor_unitario": produto.valor_unitario}


@router.delete("/produto/{id}", tags=["Produto"], status_code=200)
async def delete_produto(id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(ProdutoModel).where(ProdutoModel.id_produto == id))
    produto = result.scalars().first()
    if not produto:
        raise HTTPException(status_code=404, detail="Produto não encontrado")
